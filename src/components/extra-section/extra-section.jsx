...
const ExtraSection = ({btnStyle, children, parentId }) => {
  ...
  return (
    <div className={`${openSection ? ' duration-300' : ''}`}>
      ...
      <button onClick={() => setOpenSection(!openSection)} className={` ${btnStyle ? btnStyle : 'bg-[#1A1A1A]/50 rounded-full p-[9px] pl-[30px] flex items-center justify-between gap-6 text-currentWhiteText leading-7'} duration-300 z-20 absolute top-16 right-16 block w-fit  ${openSection ? '' : 'opacity-0'}`}>
        {t('Закрыть')}
        <div className='rounded-full w-10 lg:w-14 h-10 lg:h-14 bg-[#DB3F1C] flex items-center justify-center text-white text-3xl'>
          <IoClose />
        </div>
