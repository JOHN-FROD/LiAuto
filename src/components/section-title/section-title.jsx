const SectionTitle = ({ title, subBigTitle , subSmallTitle , styleBox , subTitleTop , darkMode , black}) => {
    ...
    return (
        <div className={` ${styleBox ? styleBox : 'text-center'}  flex flex-col gap-y-1 md:gap-y-4  ${darkMode ? 'text-white' : 'text-dark'}`}>
            ...
            {subSmallTitle &&
                <p className={`text-sm md:text-lg   ${subTitleTop ? 'order-2' : 'order-1'} ${darkMode ? 'currentWhiteText' : 'text-darkText'} ${black ? 'text-black' : ''} `}>
                    {subSmallTitle}
                </p>
            }
            {subBigTitle &&
                <p  className={`text-sm md:text-xl   text-darkText ${darkMode ? 'currentWhiteText' : 'text-darkText'} ${black ? 'text-black' : ''}`}>
                    {subBigTitle}
                </p>
            }
